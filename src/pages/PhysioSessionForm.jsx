...
const TextArea = ({ value, onChange, placeholder, rows = 4 }) => (
  <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows}
    ...
    onFocus={e => (e.target.style.borderColor = "#15213f")} onBlur={e => (e.target.style.borderColor = "#d0d0d0")} />
);
...
const OBtn = ({ children, onClick, style = {} }) => (
  <button onClick={onClick} style={{ display: "inline-flex", alignItems: "center", gap: "7px", padding: "9px 20px", backgroundColor: "#15213f", color: "#fff", border: "none", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: "pointer", boxShadow: "0 2px 8px rgba(21,33,63,0.28)", ...style }}
    ...
    <div style={{ marginTop: "8px" }}>
      {!file ? (
        <button type="button" onClick={() => ref.current.click()}
          style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 14px", backgroundColor: "#15213f", color: "#fff", border: "none", borderRadius: "6px", fontSize: "12px", fontWeight: "700", cursor: "pointer" }}>
          <Upload size={13} /> Upload
          <input ref={ref} type="file" style={{ display: "none" }} onChange={e => e.target.files[0] && onFile(e.target.files[0])} />
        </button>
      ) : (
        <div style={{ display: "inline-flex", alignItems: "center", gap: "7px", padding: "5px 10px", backgroundColor: "#eef1f7", border: "1px solid #cdd5e6", borderRadius: "6px" }}>
          <FileText size={13} style={{ color: "#15213f" }} />
...
const MultiUpload = ({ files, onAdd, onRemove, max = 4 }) => {
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "8px" }}>
      {files.map((f, i) => (
        <div key={i} style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "5px 10px", backgroundColor: "#eef1f7", border: "1px solid #cdd5e6", borderRadius: "6px" }}>
          <FileText size={12} style={{ color: "#15213f" }} />
      ...
      {files.length < max && (
        <button type="button" onClick={() => ref.current.click()}
          style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "5px 12px", backgroundColor: "#15213f", color: "#fff", border: "none", borderRadius: "6px", fontSize: "12px", fontWeight: "700", cursor: "pointer" }}>
  ...
  return (
    <div style={{ position: "relative" }}>
      <div onClick={() => setOpen(p => !p)}
        ...
        {tags.map(t => (
          <span key={t} style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "3px 9px", backgroundColor: "#eef1f7", border: "1px solid #cdd5e6", borderRadius: "20px", fontSize: "12px", fontWeight: "600", color: "#b05a00" }}>
            {t}
...
const WorkoutSelector = ({ selected, onChange }) => {
  ...
  const SectionDivider = ({ label }) => (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px" }}>
      <div style={{ width: "10px", height: "10px", borderRadius: "2px", backgroundColor: "#15213f", flexShrink: 0 }} />
    ...
    <div style={{ marginTop: "4px" }}>
        ...
        {EXERCISES_TEXT.map(ex => {
          ...
          return (
            <div key={ex.id} onClick={() => toggle(ex.id)}
              style={{ padding: "12px 14px", borderRadius: "9px", border: on ? "2px solid #15213f" : "1.5px solid #e0e0e0", backgroundColor: on ? "#eef1f7" : "#fafafa", cursor: "pointer", transition: "all 0.15s", position: "relative", boxShadow: on ? "0 2px 8px rgba(21,33,63,0.15)" : "0 1px 2px rgba(0,0,0,0.04)" }}
              onMouseEnter={e => { if (!on) { e.currentTarget.style.borderColor = "#15213f"; e.currentTarget.style.backgroundColor = "#fffaf5"; } }}
              onMouseLeave={e => { if (!on) { e.currentTarget.style.borderColor = "#e0e0e0"; e.currentTarget.style.backgroundColor = "#fafafa"; } }}
            >
              {on && (
                <div style={{ position: "absolute", top: "8px", right: "8px", width: "18px", height: "18px", borderRadius: "50%", backgroundColor: "#15213f", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                <Target size={11} style={{ color: "#15213f", flexShrink: 0 }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "10px", marginBottom: "16px" }}>
        {EXERCISES_GIF.map(ex => {
          ...
          return (
            <div key={ex.id} onClick={() => toggle(ex.id)}
              style={{ borderRadius: "9px", border: on ? "2px solid #15213f" : "1.5px solid #e0e0e0", backgroundColor: on ? "#eef1f7" : "#fff", overflow: "hidden", cursor: "pointer", transition: "all 0.15s", position: "relative", boxShadow: on ? "0 2px 10px rgba(21,33,63,0.18)" : "0 1px 3px rgba(0,0,0,0.05)" }}
              onMouseEnter={e => { if (!on) { e.currentTarget.style.borderColor = "#15213f"; e.currentTarget.style.boxShadow = "0 3px 10px rgba(0,0,0,0.09)"; } }}
            ...
            >
              <div style={{ height: "100px", backgroundColor: "#f0f0f0", position: "relative", overflow: "hidden" }}>
                ...
                {on && (
                  <div style={{ position: "absolute", top: "6px", right: "6px", width: "22px", height: "22px", borderRadius: "50%", backgroundColor: "#15213f", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.2)" }}>
                    <CheckCircle size={13} color="#fff" />
                  </div>
                )}
                <div style={{ position: "absolute", bottom: "5px", left: "5px", backgroundColor: "rgba(255,255,255,0.92)", fontSize: "9px", fontWeight: "700", padding: "2px 7px", borderRadius: "20px", color: "#15213f", border: "1px solid #cdd5e6", maxWidth: "88%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ex.muscle}</div>
              </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
              {ALL_EXERCISES.filter(e => selected.includes(e.id)).map(e => (
                <span key={e.id} style={{ fontSize: "11px", fontWeight: "600", padding: "2px 9px", borderRadius: "20px", backgroundColor: "#eef1f7", color: "#b05a00", border: "1px solid #cdd5e6" }}>{e.name}</span>
              ))}
            </div>
          </div>
        </div>
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
        <div style={{ fontSize: "13px", fontWeight: "700", color: "#333", display: "flex", alignItems: "center", gap: "7px" }}>
          <Dumbbell size={14} style={{ color: "#15213f" }} /> Exercises ({programs.length})
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {programs.map((ex, idx) => (
            <div key={ex.instanceId} style={{ border: "1.5px solid #e8e8e8", borderRadius: "10px", overflow: "hidden" }}>
              ...
              <div style={{ padding: "11px 15px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "8px" }}>
        ...
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.45)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div style={{ ...card(), width: "100%", maxWidth: "620px", maxHeight: "78vh", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "10px 20px", borderBottom: "1px solid #f0f0f0", display: "flex", gap: "7px", flexWrap: "wrap" }}>
              {["All", ...EX_CATS].map(c => (
                <button key={c} onClick={() => setCatFilter(c)} style={{ padding: "4px 11px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", cursor: "pointer", border: `1.5px solid ${catFilter === c ? "#15213f" : "#e0e0e0"}`, backgroundColor: catFilter === c ? "#eef1f7" : "#f9f9f9", color: catFilter === c ? "#15213f" : "#666" }}>{c}</button>
              ))}
            </div>
            <div style={{ overflowY: "auto", flex: 1 }}>
              {filteredLib.map((ex, i) => (
                <div key={ex.id} onClick={() => addExercise(ex)}
                  ...
                  onMouseLeave={e => (e.currentTarget.style.backgroundColor = "transparent")}
                >
                  <div style={{ width: "34px", height: "34px", borderRadius: "8px", backgroundColor: "#eef1f7", border: "1.5px solid #cdd5e6", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Dumbbell size={15} style={{ color: "#15213f" }} /></div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "13px", fontWeight: "700", color: "#222" }}>{ex.name}</div>
                    <div style={{ fontSize: "11px", color: "#888", marginTop: "1px" }}>{ex.category} · {ex.defaultReps} · {ex.defaultSets} sets</div>
                  </div>
                  <div style={{ padding: "4px 10px", backgroundColor: "#eef1f7", border: "1px solid #cdd5e6", borderRadius: "6px", fontSize: "11px", fontWeight: "700", color: "#15213f" }}>+ Add</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {clinicalGoals.length > 0 && (
            <div style={{ marginBottom: "14px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                <div style={{ width: "4px", height: "16px", backgroundColor: "#15213f", borderRadius: "2px" }} />
...
const PhysioSessionForm = ({ player, report, onBack, onSave }) => {
  ...
  const twoCol = (left, right) => (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "20px", marginBottom: "20px" }}>{left}{right}</div>
  );
  ...
  return (
    <div style={{ padding: "28px", maxWidth: "900px", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px", flexWrap: "wrap", gap: "12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
          <button onClick={onBack}
            ...
            onMouseEnter={e => (e.currentTarget.style.borderColor = "#15213f")}
            onMouseLeave={e => (e.currentTarget.style.borderColor = "#e0e0e0")}>
            <ArrowLeft size={17} style={{ color: "#555" }} />
          </button>
          <div>
            <div style={{ fontSize: "11px", color: "#15213f", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.5px" }}>Create Session</div>
            <h1 style={{ fontSize: "18px", fontWeight: "800", color: "#222", margin: 0, fontFamily: "'Playfair Display', Georgia, serif" }}>Assessment — {player?.name || "Player"}</h1>
          </div>
        </div>
        <div style={{ padding: "7px 14px", backgroundColor: "#f5f5f5", border: "1px solid #e0e0e0", borderRadius: "8px", fontSize: "13px", fontWeight: "600", color: "#555" }}>
          Date: <span style={{ color: "#15213f" }}>{today}</span>
        </div>
      </div>
      ...
      <div style={cardStyle}>
        ...
        <button style={{ display: "inline-flex", alignItems: "center", gap: "7px", padding: "9px 18px", backgroundColor: "#15213f", color: "#fff", border: "none", borderRadius: "7px", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: "0", borderBottom: "2px solid #f0f0f0", marginBottom: "20px" }}>
          {[
            ...
            <button key={tab.key} onClick={() => setSessionTab(tab.key)}
              style={{ display: "flex", alignItems: "center", gap: "6px", padding: "10px 18px", border: "none", cursor: "pointer", fontSize: "13px", fontWeight: "700", backgroundColor: "transparent", color: sessionTab === tab.key ? "#15213f" : "#888", borderBottom: sessionTab === tab.key ? "2px solid #15213f" : "2px solid transparent", marginBottom: "-2px", transition: "all 0.15s" }}>
